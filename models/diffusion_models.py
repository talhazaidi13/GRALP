...
class ResBlock(nn.Module):
    ...
    def forward(self, x, t, s):
        t_embed = self.time_layer(t)    #  torch.Size([32, 256])   ->   torch.Size([32, 256])
        s_embed = self.layer_norm(self.state_layer(s))   #  torch.Size([32, 256])   ->   torch.Size([32, 128]) 
        h = torch.cat([x,s_embed], dim=1)  # torch.Size([32, 128])+torch.Size([32, 128]) -> torch.Size([32, 256])
        h = self.layer1(h)               # torch.Size([32, 256]) -> torch.Size([32, 128])
        h = self.layer_norm(h)              #torch.Size([32, 128]) -> torch.Size([32, 128])   
        scale_shift = t_embed.chunk(2, dim=1)   # used to split the tensor t_embed into two equal parts   -> [ torch.Size([32, 128]) , torch.Size([32, 128]) ]  sin, cos
        scale, shift = scale_shift       # [Scale uses sin = torch.Size([32, 128]) ,  shift uses cos = torch.Size([32, 128]) ]
        h = h * (scale + 1) + shift   # torch.Size([32, 128])
        h = self.act(h)             # torch.Size([32, 128])
        h = self.layer2(h)          # torch.Size([32, 128])
        h = self.layer_norm(h)   # torch.Size([32, 128])
        h = self.act(h)
        h = h + x            # Residual connection, latent state added into the output     # torch.Size([32, 128])
        return h
...
class UNet(nn.Module):
    ...
    def forward(self, x, t, s):  # y,t,x   ; x=latent_state, t=time, s=orignal_stae      ## # #y_i=[128000, 16], t_is=100*[128000, 1], x_batch=[128000, 29]
        t_embed = self.time_mlp(t)
        s_embed = self.state_mlp(s)
        x = self.init_layer(x)
        #Down
        h1 = self.res_block1(x, t_embed, s_embed)      #   h1=torch.Size([32, 128])   
        h2 = nn.GELU()(self.layer_norm2(self.down1(h1)))  #  self.down1(h1).shape = [32,32]  , h2shape = [32,32]
        h2 = self.res_block2(h2, t_embed, s_embed)       #  h2=torch.Size([32, 32])
        h3 = nn.GELU()(self.layer_norm3(self.down2(h2)))  #  self.down2(h2).shape = [32,16]  , h3shape = [32,16]
        h3 = self.res_block3(h3, t_embed, s_embed)       #  h3=torch.Size([32, 16])
        h4 = nn.GELU()(self.layer_norm4(self.down3(h3))) #  self.down3(h3).shape = [32,8]  , h4shape = [32,8]
        h4 = self.res_block4(h4, t_embed, s_embed)     #  h4=torch.Size([32, 8])
        #Up
        h = nn.GELU()(self.layer_norm3(self.up1(h4)))  #  self.up1(h4).shape = [32,16]  , hshape = [32,16]
        h = h+h3                                       #  additive link in UNET = h.shape = [32,16]
        h = self.res_block5(h, t_embed, s_embed)       #  h=torch.Size([32, 16])
        h = nn.GELU()(self.layer_norm2(self.up2(h)))    #  self.up2(h).shape = [32,32]  , hshape = [32,32]
        h = h+h2                                        #  additive link in UNET = h.shape = [32,32]
        h = self.res_block6(h, t_embed, s_embed)        #  h=torch.Size([32, 32])
        h = nn.GELU()(self.layer_norm1(self.up3(h)))    #  self.up3(h).shape = [32,128]  , hshape = [32,128]
        h = h+h1                                        #  additive link in UNET = h.shape = [32,128]
        h = self.res_block7(h, t_embed, s_embed)        #  h=torch.Size([32, 128])
        h = h+x                                         #  additive link in UNET = h.shape = [32,128]
        h = self.final_res_block(h, t_embed, s_embed)   #  h=torch.Size([32, 128])
        h = self.final_mlp(h)                           #  h=torch.Size([32, 16])
...
class Model_mlp_diff_embed(nn.Module):
    ...
    def forward(self, y, x, t, context_mask):
        # embed y, x, t
        if self.net_type == 'unet':
            net_output = self.unet(y, t, x)   # netoutput=[32,16], y=[32,16], t=[32,1], x[32,29]     # #y_i=[128000, 16], t_is=100*[128000, 1], x_batch=[128000, 29]
        else:
            if self.use_prev:
...
def ddpm_schedules(beta1, beta2, T, schedule):  # ddpm_schedules(betas[0], betas[1], n_T=500, schedule)
    """
    ...
    alphabar_t = torch.cumsum(log_alpha_t, dim=0).exp()   # calculates alphabar = \prod_{i=1}^{t} \alpha_i = exp(\sum_{i=1}^{t} log(\alpha_i))
...
class Model_Cond_Diffusion(nn.Module):
    ...
    def loss_on_batch(self, x_batch, y_batch, predict_noise=True):     # self, x0, latent_z0, predict_noise=True
        _ts = torch.randint(1, self.n_T + 1, (y_batch.shape[0], 1)).to(self.device)  # sample random timesteps between 1 and n_T=500 od size (batchsize=32 ,1)

        # dropout context with some probability # produces binary outcomes (0 or 1) with a probability p. For each element in the tensor, it generates a 1 with probability self.drop_prob, and a 0 with probability 1 - self.drop_prob
        ...
        y_t = self.sqrtab[_ts] * y_batch + self.sqrtmab[_ts] * noise    # Forward diffusion step adding noise to clean target actions y_t=noisylatent zt [128,16]
        if self.normalize_latent:
            y_t = y_t/torch.norm(y_t, dim=-1).unsqueeze(-1)

        # use nn model to predict noise
        if self.nn_model.net_type == 'unet':
            noise_pred_batch = self.nn_model(y_t, x_batch, _ts, context_mask)    # noisylatent_zt_[128,16], orig_s0_[128,29], _ts=noise schedule
        else:
    ...
    def sample_extra(self, x_batch, extra_steps=4, return_y_trace=False, predict_noise=True): #xbaatch = [ending traj states xt - repeat -> size 256*500, 29] 
        # also use this as a shortcut to avoid doubling batch when guide_w is zero
        is_zero = False
        if self.guide_w > -1e-3 and self.guide_w < 1e-3:
            is_zero = True
        ...
        # how many noisy actions to begin with
        n_sample = x_batch.shape[0]    # 256*500=128000

        y_shape = (n_sample, self.y_dim) # [128000, 16]

        # sample initial noise, y_0 ~ N(0, 1),
        y_i = torch.randn(y_shape).to(self.device) # [128000, 16]   
        ...
        if not is_zero:
            ...
            context_mask = torch.zeros(x_batch.shape[0]).to(self.device)  # zeros[128000]
        ...
        for i_dummy in range(self.n_T, -extra_steps, -1):
            ...
            z = torch.randn(y_shape).to(self.device) if i > 1 else 0       # z=[128000, 16]  sample initial noise, y_0 ~ N(0, 1),

            # split predictions and compute weighting
            if self.nn_model.net_type == 'unet':  #out eps = [128000, 16]
                eps = self.nn_model(y_i, x_batch, t_is*self.n_T, context_mask)  #y_i=[128000, 16]=latent_noise_undergodiff, x_batch=st-interleaved diffsteps[128000, 29], t_is=100*[128000, 1], context_mask=zeros[128000]
                #noise_pred_batch = self.nn_model(y_t, x_batch, _ts, context_mask)    # noisylatent_zt_[128,16], orig_s0_[128,29], _ts=noise schedule
            else:
                eps = self.nn_model(y_i, x_batch, t_is, context_mask)
            ...
            if not predict_noise:
                x0 = eps                         # neural network predicts noise 𝜖 added to the data during the forward process.
                eps = 1 / self.sqrtmab[i] * (y_i - self.sqrtab[i] * x0)  # To compute the next state, this noise 𝜖  must be subtracted from the current state 𝑦𝑖 to denoise it. 
                 # predicted clean data x0 = 1/sqrt(alphabar) * (yt - sprt(1-alphsbart)*𝜖 )
